...
#include <veekay/stb_image/stb_image.h>
...
// Helper function to begin single-time command buffer
...
// Helper function to end single-time command buffer
...
// Helper function to create VkImage
...
// Helper function to create VkBuffer
void Texture::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void Texture::loadImage(const std::string& filepath) {
    int texChannels;
    stbi_uc* pixels = stbi_load(filepath.c_str(), (int*)&width, (int*)&height, &texChannels, 4); // ИЗМЕНЕНО: STBI_RGBA на 4
    if (!pixels) {
    ...
    createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, // ИЗМЕНЕНО: используем createBuffer
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                 stagingBuffer, stagingBufferMemory);

    void* data;
    vkMapMemory(veekay::app.vk_device, stagingBufferMemory, 0, imageSize, 0, &data);
    memcpy(data, pixels, static_cast<size_t>(imageSize));
    vkUnmapMemory(veekay::app.vk_device, stagingBufferMemory);
    ...
    vkDestroyBuffer(veekay::app.vk_device, stagingBuffer, nullptr);
    vkFreeMemory(veekay::app.vk_device, stagingBufferMemory, nullptr);
}
...
void Texture::createSampler() {
    ...
    if (vkCreateSampler(veekay::app.vk_device, &samplerInfo, nullptr, &sampler) != VK_SUCCESS) {
